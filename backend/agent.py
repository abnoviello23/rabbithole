...
path_to_project = os.getcwd() + "/project"

mcp_servers = {
    "exa": {
        "type": "http",
        "url": "https://mcp.exa.ai/mcp?exaApiKey=0c577dcf-7e0a-43c8-ab25-d2d4643bf856",
    },
   
    "openai-summary-tools": summary_tools_server,  # Custom OpenAI summary tools
}
...
{chr(10).join([f"- {ctx.get('title', '')}: {ctx.get('content', '')}" for ctx in context.values()])}
...
Remember: Research iteratively, explore deeply, then deliver audio insights.""",
    ...
    async with ClaudeSDKClient(options=options) as client:
        ...
        # First query
        await client.query("i wanna know how zohran mamdani won the election")
        async for message in client.receive_response():
            messages.append(message)
